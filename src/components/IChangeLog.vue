    ...
    <template v-if="historyList.length > 0">
      ...
      >
        <div class="history-item-inner">
          <div class="bar">
            <span>{{ item.date }}</span> <span>{{ item.title }}</span>
          </div>
          <div class="content">
            <div class="left">
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      // } else {
      IDM.http
        .get("ctrl/szydLibrary/getRecord", {
          ...IDM.url.queryObject(),
        })
        .done((res) => {
          this.historyList = res.data.map((item) => ({
            date: item.operateTime,
            title: item.operateField,
            before: item.originalcontent,
            after: item.updateContent,
          }));
        })
        .error((response) => {});
      // }
